...
abstract class ProtobufController[TRequest <: GeneratedMessage with Message[
  TRequest]: GeneratedMessageCompanion, TResponse <: GeneratedMessage with Message[
  TResponse]: GeneratedMessageCompanion](cc: ControllerComponents)
  ...
  def protoAction() = Action.async(new ProtoParser) {
    implicit request: Request[TRequest] =>
  ...
  private class ProtoParser extends BodyParser[TRequest] with Results {
    ...
    override def apply(header: RequestHeader)
      : Accumulator[ByteString, Either[Result, TRequest]] = {
      header.contentType map {
        case "application/x-protobuf" => Accumulator(bytesSink)
        case "application/json" => Accumulator(jsonSink)
        case _ =>
          Accumulator.done(
            Left(BadRequest("API only supports protobuf as JSON or bytes.")))
      } get
    }
  }
}
